...
По умолчанию пушить в `main` запрещено.
Исключение допустимо только если пользователь **явно и однозначно** дал команду на конкретный push/merge в `main` в текущем диалоге.
...
### 1.5 Merging
- По умолчанию агент **не мержит** PR.
- Merge в `main` допустим только по **прямой и однозначной команде пользователя** на конкретный PR или конкретную операцию merge.
